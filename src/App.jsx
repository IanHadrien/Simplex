...
export default function App() {
      ...
      if (newForm.constrained[i].to == '>=') {
        ...
        for (let j = 0; j < newLinhas.length; j++) {
          ...
          if (newForm.constrained.indexOf(newForm.constrained[i]) + 1 == j) {
            ...
            newIndexExecessos.push(
              newLinhas[0].indexOf(newLinhas[0][newLinhas[j].length - 1]),
            );
          }
        }
      }
    ...
    if (newDuasFases) {
      ...
      for (let i = 0; i < newLinhas.length; i++) {
        if (newLinhas[i][1] == 1) {
          ...
          newLinhas[0][newLinhas[0].length - 1] +=
            newLinhas[i][newLinhas[0].length - 1];
        }
      }
    }
  ...
  const iteracao = (
    ...
    for (let i = 1; i < novaLinha.length; i++) {
      if (novaLinha[i][novaLinha[i].length - 1] == 0) {
        ...
        if (iteracaoExcesso == false) {
          divisao.push(
            parseFloat(novaLinha[i][novaLinha[i].length - 1]) /
              linhaAnterior[i][queEntra + 2],
          );
        } else {
          divisao.push(
            parseFloat(novaLinha[i][novaLinha[i].length - 1]) /
              linhaAnterior[i][queEntra],
          );
        }
      }
    }
    ...
    for (let i = 2; i < novaLinha[queSai].length; i++) {
      novaLinha[queSai][i] =
        linhaAnterior[queSai][i] / linhaAnterior[queSai][queEntra];
    }
    ...
    for (let i = 0; i < novaLinha.length; i++) {
      for (let j = 2; j < novaLinha[queSai].length; j++) {
        if (i != queSai) {
          novaLinha[i][j] =
            linhaAnterior[i][j] -
            linhaAnterior[i][queEntra] * novaLinha[queSai][j];
        }
      }
    }
    ...
    if (countNovaLinha0 < 0 || newCountExecessos > 0) {
      ...
      for (let i = 2; i < linhaZduasFases[0].length; i++) {
        ...
        for (let j = 1; j < linhaZduasFases.length; j++) {
          linhaZduasFases[0][i] +=
            linhaZduasFases[j][1] * linhaZduasFases[j][i];
        }
      }
    ...
    if (maisiteracoes) {
      ...
      iteracao(
        newInteracoes[newInteracoes.length - 1],
        newCountExecessos,
        newIndexExecessos,
        newInteracoes,
        newDuasFases,
      ...
      // ITERAÇÃO FINAL
      let iteracaoFinal = JSON.parse(
        JSON.stringify(newInteracoes[newInteracoes.length - 1]),
      );
      ...
      for (let j = 0; j < newForm.f.length; j++) {
        ...
        if (aux == 1) {
          for (let i = 1; i < novaLinha.length; i++) {
            if (novaLinha[i][0].includes('x_' + counter)) {
              newResultado.xOtimo.push(
                iteracaoFinal[i][iteracaoFinal[i].length - 1],
              );
            }
          }
        } else {
  ...
  const gerarGraficos = (array, xOtimo, zOtimo) => {
    ...
    axios
      ...
      .then((response) => {
        setImagem('./assets/function_chart.svg');
  ...
  const calcIntegers = () => {
    ...
    for (let i = 0; i < combinacoes.length; i++) {
      if (wasMIN) {
        ...
        if (
          resultadoInteiro >= resultado.zOtimo &&
          resultadoInteiro < zInteiro
        ) {
      ...
      <div className="w-3/4 m-auto py-6">
        <div className="w-1/2 m-auto">
          ...
          <div className="flex justify-center">
            <div>
              ...
              <div className="mt-2 flex justify-center">
                <button
                  onClick={subVars}
                  className="text-red-600 border-2 mr-2 hover:bg-gray-50"
                >
                  <IoIosRemove size={30} />
                </button>
                <button
                  onClick={addVars}
                  className="text-blue-600 border-2 hover:bg-gray-50"
                >
                  <IoIosAdd size={30} />
                </button>
              </div>
            ...
            <div className="pl-4">
              ...
              <div className="mt-2 flex justify-center">
                <button
                  onClick={subConstraineds}
                  className="text-red-600 border-2 mr-2 hover:bg-gray-50"
                >
                  <IoIosRemove size={30} />
                </button>
                <button
                  onClick={addConstraineds}
                  className="text-blue-600 border-2 hover:bg-gray-50"
                >
                  <IoIosAdd size={30} />
                </button>
                        ...
                        <td>
                          <FormControl sx={{ minWidth: 80 }}>
                            <Select
                              ...
                              onChange={(e) =>
                                editRestricoes(e.target.value, index)
                              }
                              displayEmpty
                            >
          ...
          <div className="mb-12 mt-5">
            ...
            <button
              className="bg-red-600 text-white p-2 w-full rounded my-4"
        ...
        <div>
          {interacoes?.length > 0 && (
            <div>
              <p className="text-xl">
                A solução ótima é{' '}
                <span className="font-bold">Z = {resultado?.zOtimo}</span>
              </p>
            </div>
          )}
          ...
          <div className="pt-4">
            {interacoes?.length > 0 && (
              <div>
                <p className="text-xl">
                  A solução inteira é{' '}
                  <span className="font-bold">Z = {resultado?.zInteger}</span>
                </p>
              </div>
            )}
